...
pub struct EngineStarter {
    ...
    cached_starters: HashMap<u64, Vec<usize>>,
    rand: StdRng,
    max_len: usize,
}
...
impl EngineStarter {
    pub fn new(num_hash: usize, plane_dim: usize, graph_size: usize, seed: Option<u64>) -> Self {
        ...
        Self {
            hasher: hasher,
            cached_starters: HashMap::new(),
            rand: rng,
            max_len: graph_size,
        }
    }

    pub fn select_starting_points(&mut self, query: &[f32], k: usize) -> Vec<usize> {
        let signature = self.hasher.hash_int(query);
        if let Some(starters) = self.cached_starters.get(&signature) {
            starters.clone()
        } else {
            let indices = sample(&mut self.rand, self.max_len, k).into_vec();
            self.cached_starters.insert(signature, indices.clone());
            indices
        }
    }
}
